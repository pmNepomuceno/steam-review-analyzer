...
- **ML pipeline (`backend/src/ml/`):**
  ...
  - `aspects.py` and `anchors.py`: reviews are split into sentences, embedded with sentence-transformers, and assigned to an aspect by cosine similarity to hand-written anchor phrases (or to "none" below a threshold). Anchor-based assignment is deliberately chosen over BERTopic or a fine-tuned classifier.
